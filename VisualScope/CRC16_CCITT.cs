...
namespace 地面站
{
    ...
    /// </summary>  
    public static partial class CRC16_CCITT
	{
		...
		public static UInt16 Calculate(Byte[] data, int data_Len)
		{
			UInt16 crc = 0xffff;
            int i = 0;
			while ((data_Len--) > 0)
			{
				crc =(UInt16)(CRC16Table[(crc >> 8 ^ data[i++]) & 0xff] ^ (crc << 8));
            }
			return crc;
		}
		#endregion CRC16-CCITT(初始值FFFF，多项式1021，异或值FFFF，异或输出，表逆序，算法逆序)
	}
}
